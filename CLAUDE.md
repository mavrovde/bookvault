...
**One backend state machine, split by domain.** `web/bookvault_web/activity/` is a package: `state.py` (the machine — states, the shared `_state` dict, `_begin`/`cancel`), `library.py` (REFRESHING + CHECKING), `mirror.py` (DOWNLOADING, and the one answer to "do I already have this?"), `archive.py` (PREPARING), `abs_sync.py` (SYNCING), with `__init__.py` as the façade `app.py`/`autosync.py` import. Only one activity runs at a time (falls out of the single worker thread). **Cross-module references go through the module object** (`library._iter_books(...)`, `state._state`), never `from .library import _iter_books` — a `from` import binds at import time, so a `monkeypatch.setattr` on the owning module wouldn't reach the caller and the test would pass while injecting nothing. For the same reason tests patch the module that *owns* a name (`activity.library.PACE_SECONDS`), and the façade deliberately doesn't re-export patchable internals, so a mis-aimed patch raises `AttributeError` instead of silently doing nothing. The browser is a thin renderer: it POSTs an action and polls `GET /activity`. A finished build's per-book results and its zip link are kept **durably** (`results`, `zip_path`, `saved_path`, untouched by `_begin`) so they survive the size-check that fires on the next page load.

**The zip is staged in temp, then moved.** A build always writes into its own `mkdtemp` workdir and the finished archive is moved into the user's save folder (`prefs.resolve_download_dir()`, passed in by `app.py` — `activity/` never imports `prefs`) only *after* it succeeds, so a crashed or empty build never leaves a half-written `.zip` in someone's Downloads. A failed move keeps the archive in temp and still serves it; the archive is never discarded. Cleanup tracks `_state["workdir"]` explicitly — **never** derive the temp dir from `Path(zip_path).parent`, because once saved that parent is the *user's own folder*.
...
**"Do I already have this?" is answered from our own record, never litres.ru's.** The catalogue's `size` for a file is *not* the byte length the site serves — across a real 239-book library, 10 matched and 147 did not. Any completeness check against it re-downloads nearly everything on every run, which is the request pattern that gets an account anti-bot flagged. So a finished download records what it actually wrote in `.bookvault-index.json` beside the files (`library_fs.record_in_mirror_index`), and completeness compares against *that*: byte length for an ebook, track count for an unpacked audiobook. No record means nothing to verify — a present file is **trusted**, not re-fetched, so a library the user assembled by hand isn't destroyed by a first run. Three consumers ask this question — the mirror loop, the ✓ badge (`books_on_disk`), and the zip's reuse (`_local_copy_for`) — and all three must go through `_is_on_disk`; MCP's `download_book` uses the same index. They were once implemented separately and silently drifted. Listing sizes remain fine for *display* and for the `~` progress estimate, which is all they can honestly support.
...
web/bookvault_web/     app.py (FastAPI) · prefs.py (shared UI state) · folder_dialog.py (native save-folder picker) · run.py · templates/ static/
                       activity/  state.py (the machine) · library.py (refresh/sizes) ·
                                  mirror.py (loose-file mirror) · archive.py (zip build) ·
                                  abs_sync.py (Audiobookshelf tree) · __init__.py (façade)
...
Versions are bumped **in lockstep** across all five `pyproject.toml` files (root + core/web/mcp/desktop); current release is **v1.4.0**. A pushed `v*` tag triggers `.github/workflows/docker-publish.yml`, which builds and publishes the multi-arch `ghcr.io/mavrovde/bookvault/{web,mcp}` images. `.github/workflows/lint-test-audit.yml` runs ruff + the pytest matrix (3.11–3.13) + a dependency audit on every push/PR.
