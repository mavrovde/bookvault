...
- **Session restore is best-effort by contract** (`session.py`): it runs unattended at boot, so *any* failure (unreachable litres.ru, Playwright timeout) must log, close the client, keep the unvalidated session file, and leave the app logged out — never crash the boot or leak a Chromium. Interactive `login()` is the opposite: it fails loudly, wrapping unexpected errors as `LitresAuthError` after closing the client, and rejects a "successful" login that captured no app-level headers. `LitresClient.__init__`/`close()` unwind partial startup / teardown failures so the driver process is never orphaned.
- **Persisted JSON state is written atomically** — `cache.py` and `prefs.py` save via write-tmp + `os.replace`, and the session file is chmod'd `0600`. Follow the same pattern for any new persisted state.
